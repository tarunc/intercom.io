...
Intercom.prototype.getPages = function(path, parameters, cb) {
  ...
  // TODO Limit concurrency, Q can fail when making hundreds of requests in parallel
  // TODO Add rate limiting (max 500 req/min to Intercom)
  var self = this;
  ...
  return rootPromise.then(function (root) {
    return root.pages.total_pages;
  }).then(function (totalPages) {
    return Q.all(allPagesPromises(totalPages));
...
Intercom.prototype.markConversationAsRead = function (convObj, cb) {
  if (convObj && convObj.hasOwnProperty('id')) {
    return this.request('PUT', 'conversations/' + convObj.id + '/reply', convObj, cb);
  } else {
    return this.request('PUT', 'conversations/last/reply', convObj, cb);
  }
};
